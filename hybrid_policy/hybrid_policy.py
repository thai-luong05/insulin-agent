...
def run_hybrid_episode(patient_id, drl_policy=None, mpc_sigma_frac=0.15,
                       overrides=None, seed=0, verbose=True):
    # 24h rollout: MPC sets the dose, DRL nudges it via the blend, meal bolus is a floor; smaller mpc_sigma_frac = trust MPC more, drl_policy=None = pure MPC
    from main import (get_clinical_params, get_body_weight, get_patient_env,
                      get_env, custom_reward, Options, Pump, get_basal,
                      BergmanMPC, default_max_bolus_mult, calibrate, PREBOLUS_STEPS)
    ...
    mpc = BergmanMPC(basal_rate=basal, isf=isf, basal_bg=bg0, body_weight=bw,
