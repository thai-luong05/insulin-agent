...
def setup_experiment_dir(args):
    experiment_dir = os.path.join(os.path.dirname(__file__), 'results_ppo', args.folder_id)
...
def train():
    args = Options().parse()
    args = set_args(args)  # apply G2P2C's PPO hyperparameters
    ...
    # create the G2P2C PPO agent
    agent = PPO(args, device, load=False, path1='', path2='')
    ...
    # meal_prob: first slot is calibration window (~30 min), keep it -1 (no meal)
    # so calibration never hits a meal and avoids infinite reinit loop
    ...
    print(f"Starting training for up to {MAX_INTERACTIONS} interactions...")
    ...
    for rollout in range(30000):
        ...
        # PPO worker.rollout returns data dict directly (not tuple)
        for i in range(args.n_training_workers):
            data = worker_agents[i].rollout(agent.policy)
            agent.old_states[i] = data['obs']
            agent.feat[i] = data['feat']
        ...
        ri = 0
        for i in range(args.n_testing_workers):
            res = testing_agents[i].rollout(agent.policy)
            ri += res[0]
...
def test(agent, reward_history, testing_args):
    ...
    fig.suptitle(f'Glucose Regulation \u2014 Patient {patient_id} (PPO)', fontsize=14, fontweight='bold')

    # ---- subplot 1: training reward ----
    ax1 = axes[0]
    rewards_arr = np.array(reward_history)
    ax1.plot(rewards_arr, alpha=0.3, color='steelblue', linewidth=0.5)
    window = max(min(len(rewards_arr) // 20, len(rewards_arr)), 1)
    if len(rewards_arr) >= window:
        ...
        ax1.plot(rewards_arr, color='darkblue', linewidth=2, label='Reward')
    ax1.set_title(f'Patient {patient_id} \u2014 Training Reward (PPO)', fontsize=11)
    ...
    plt.tight_layout()
    plt.savefig('simulation_result_ppo.png', dpi=150, bbox_inches='tight')
    plt.show()
